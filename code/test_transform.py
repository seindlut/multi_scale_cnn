...
def test_cifar10(datapath, testset_name, param_file_name, nkerns=[32,32,64], batch_size=10000):
    ...
    classifier_params = unpickle('parameters_trans')
    ...
    trans_mat = unpickle('transformation_matrix')
    trans_mat = theano.shared(trans_mat)
    class_layer0_input = T.dot(fc_layer0.output,  trans_mat.T) 

    class_layer0 = LogisticRegression(
        input=class_layer0_input, 
        n_in=fc_layer0_hidden_nodes, 
        n_out=10,
        W=classifier_params[0].get_value(),
        b=classifier_params[1].get_value()
    )
